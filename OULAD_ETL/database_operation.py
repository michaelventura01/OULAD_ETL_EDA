...
class Database_Operation:
    ...
    def __get_sakila_string(self):
        config = Config.get_config()['oulad']
    ...
    def registro_carga_sakila(self, esquema, vista, registros):
        ...
        try:
            ...
            conexion.commit()
            print(f"\t |> Reporte {vista.replace('vw_', '')} CARGADO en {esquema} en oulad con {registros} registros | {_fecha}")
        except Exception as e:
            print(f"\t |> Reporte {vista.replace('vw_', '')} cargado en {esquema} No fue CARGADO en oulad | {_fecha}", e)
            conexion.rollback()
        finally:
            cursor.close()
            conexion.close()
